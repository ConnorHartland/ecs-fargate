...
### View test logs
```bash
aws logs tail /aws/codebuild/ecs-fargate-develop-service-2-e2e-tests --follow
```

### Check pipeline status
```bash
aws codepipeline get-pipeline-state --name ecs-fargate-develop-service-2-pipeline
```

### Manually trigger tests
```bash
aws codepipeline start-pipeline-execution --name ecs-fargate-develop-service-2-pipeline
```
